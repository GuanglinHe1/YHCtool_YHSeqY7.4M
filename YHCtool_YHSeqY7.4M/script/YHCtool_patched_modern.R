...
#   - input_table: TSV with columns [sample_id, haplogroup_full, population] (order flexible; auto-detected)
# Optional inputs:
...
# Set this to the folder containing this script, e.g.:
# setwd("/path/to/your/project/script")
# --- Input files ---
if (!exists("input_table")) input_table <- "../data/Population.tsv"
if (!exists("meta_csv"))   meta_csv   <- "../conf/group_meta.csv"    # optional
if (!exists("ystr_csv"))   ystr_csv   <- "YSTR_table.csv"             # unused unless this file exists
...
if (!exists("root_out"))               root_out <- "../output"
...
if (!exists("bootstrap_n_per_sample")) bootstrap_n_per_sample <- 10000
if (!exists("bootstrap_n_reps"))       bootstrap_n_reps       <- 1000
...
# --- Dependencies ---
need_pkgs <- c(
  "dplyr","tidyr","stringr","ggplot2","ggrepel",
...
safe_read_input <- function(path){
  ...
  if (any(is.na(c(id_idx, hap_idx, pop_idx)))) {
    if (length(cn) >= 3 && all(grepl("^v[0-9]+$", cn[1:3], ignore.case = TRUE))) {
      ...
      if (ncol(df) < 3) stop("Input table must contain at least: sample_id, haplogroup_full, population")
      colnames(df)[1:3] <- c("sample_id","haplogroup_full","population")
    }
...
simpson <- function(p){ 1 - sum(p^2) } #  Simpson's Index of Diversity (i.e., 1 - sum(p^2))
...
plot_dendrogram <- function(D, out_file){
  ...
  pdf(paste0(base, ".pdf"), width=plot_base_width, height=plot_base_height); plot(hc, xlab="", sub="", main="Hierarchical clustering (ward.D2)"); dev.off()
  png(paste0(base, ".png"), width=plot_base_width*100, height=plot_base_height*100); plot(hc, xlab="", sub="", main="Hierarchical clustering (ward.D2)"); dev.off()
...
plot_nj_tree <- function(D, out_file, title="Neighbor-Joining tree"){
  ...
  pdf(paste0(base, ".pdf"), width=plot_base_width, height=plot_base_height); plot(tr, main=title, cex=0.8); dev.off()
  png(paste0(base, ".png"), width=plot_base_width*100, height=plot_base_height*100); plot(tr, main=title, cex=0.8); dev.off()
...
make_multilayer_long <- function(df, HH_chain = c(1,2,3,4)){
  tmp <- df
  for (k in HH_chain) tmp[[paste0("hap", k)]] <- hap_substr(tmp$hap_clean, k)
  axes <- c(paste0("hap", HH_chain), "population")
  long <- tmp %>% dplyr::count(dplyr::across(dplyr::all_of(axes)), name = "n") %>%
    dplyr::group_by(population) %>% dplyr::mutate(freq = n / sum(n)) %>% dplyr::ungroup()
  for (ax in axes) long[[ax]] <- as.character(long[[ax]])
  stopifnot(all(vapply(long[axes], is.character, TRUE))); long$population <- as.character(long$population)
  deepest_col    <- axes[length(axes) - 1]; deepest_levels <- sort(unique(long[[deepest_col]]))
  ...
  hap_axes <- axes[-length(axes)]; n_hap_axes <- length(hap_axes)
  pal_all <- pal_deep
  if (n_hap_axes > 1) {
  ...
  list(long=long, axes=axes, deepest=deepest_col, deepest_levels=deepest_levels, pal_all=pal_all, pop_levels=pop_levels)
}
...
# --- Main ---
message2("Reading input: %s", input_table)
df0 <- safe_read_input(input_table)
...
max_hh <- max(nchar(df_in$hap_clean), na.rm=TRUE)
HH_range <- HH_range[HH_range <= max_hh]
if (length(HH_range)==0) stop("All HH_range values exceed observed hap lengths; check input.")
...
for (HH in HH_range){
  ...
  df <- df_in %>% mutate(hap = hap_substr(hap_clean, HH))
